...
class ReAllocation(GateAllocation):
    ...
    def get_variable(self):
        for inst in self.interval:
            ...
            if fix_gate:
                self.model.addVar(vtype=gurobipy.GRB.BINARY, name=f"x_{inst}_{fix_gate[0]}")
                self.available_gate_dict[inst] = [fix_gate[0]]
            else:
    ...
    def _get_move_cost(self, inst: IntervalBase, ag: str) -> float:
        alpha = 1000 * 1000
        init_gate = get_fixed_result(inst, self.init_results, inst.begin_callsign[-2:])
        last_gate = get_fixed_result(inst, self.last_results, inst.begin_callsign[-2:])
...
def cost_for_international(ag: str, last_gate: list, alpha: int) -> float:
    if ag == last_gate[0]:
        return 0
    return 1 * alpha


def cost_for_cargo(ag: str, last_gate: list, alpha: int) -> float:
    if ag == last_gate[0]:
        return 0
    return 10 * alpha


def cost_for_domestic(ag: str, init_gate: list, last_gate: list, alpha: int) -> float:
    if find_group(last_gate[0]) == "TERMINAL_ONE" or find_group(last_gate[0]) == "TERMINAL_TWO":
        if ag == last_gate[0]:
            return 0
        if ag in GROUP_DICT[find_group(last_gate[0])]:
            return 1 * alpha
        if ag in GROUP_DICT["REMOTE_ONE"] or ag in GROUP_DICT["REMOTE_TWO"]:
            return 100 * alpha
        return 10 * alpha

    if find_group(init_gate[0]) == "TERMINAL_ONE" or find_group(init_gate[0]) == "TERMINAL_TWO":
...
def fixed_result(inst: IntervalBase, quarter: int, last_results: dict) -> Union[None, list]:
    """
    ...
    if inst.begin_callsign == inst.end_callsign:
        ...
        if inst_type == "de" and inst.time_dict[inst_type]["ATOT"] < quarter * 15 * 60 + 30 * 60:
            result = get_fixed_result(inst, last_results, inst_type)
        ...
        if inst.time_dict[inst.begin_callsign[-2:]]["ALDT"] < quarter * 15 * 60 + 30 * 60:
            result = get_fixed_result(inst, last_results, inst.begin_callsign[:2])
        if inst.time_dict[inst.end_callsign[-2:]]["ATOT"] < quarter * 15 * 60 + 30 * 60:
            result = get_fixed_result(inst, last_results, inst.end_callsign[:2])
    assert result is None or len(result) == 1
    return result


def get_fixed_result(inst: IntervalBase, last_results: dict, inst_type: str) -> list:
    """
    从过去的结果中找到 inst 对应的机位
    """
    result = []
