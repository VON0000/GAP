...
\begin{document}
	\begin{algorithm}
		...
		\begin{algorithmic}[1]
			...
			\Statex \Comment{Data within 12pm to 1am is based on the actual time, while the rest uses target time }
	...
	\begin{algorithm}
		...
		\begin{algorithmic}[1]
			...
			\Ensure $fixList$
			\State \Comment{$intervalData$ within the next 30 minutes from the current $quarter$ use the gate allocated in the previous iteration}
			\State $m \gets$ length of $intervalData$ 
			\For{$i \gets 0$ to $m-1$}
